...
$("#productMultiSlider")
    ...
    .each(function () {
        ...
        var $mainSlider = $(".slider_" + product_id + "_" + variant_id + ".main-slider");
        var $navSlider = $(".slider_" + product_id + "_" + variant_id + "_nav.thumbnail-slider");
        $mainSlider.slick({
            slidesToShow: 1,
            slidesToScroll: 1,
            arrows: false,
        ...
        $navSlider.closest('.slick-slider').css('margin-top', '30px');
    });
...
$("#productDetail").on("click", ".variant", function () {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            $("#overlay").fadeIn(300);
            $("#productMultiSlider").find(".productMultiSlider").addClass("hide");
        },
        success: function (data) {
            if (data.res == "success") {
                $("#discount").html(data.html);
                // $("#productMultiSlider").find(".slider_" + product_id + "_" + variant_id).removeClass("hide").slick("refresh");
                var $slider = $("#productMultiSlider").find(".slider_" + product_id + "_" + variant_id);
                $slider.removeClass("hide");
                var mainSelector = $(".slider_" + product_id + "_" + variant_id + ".main-slider");
                var navSelector = $(".slider_" + product_id + "_" + variant_id + "_nav.thumbnail-slider");
                if ($(mainSelector).length && $(mainSelector).hasClass("slick-initialized")) {
                    $(mainSelector).slick("unslick");
                }
                if ($(navSelector).length && $(navSelector).hasClass("slick-initialized")) {
                    $(navSelector).slick("unslick");
                }
                $(mainSelector).slick({
                    slidesToShow: 1,
                    slidesToScroll: 1,
...
$("#checkoutSubmit").click(function () {
    ...
    var billing_first_name = $.trim(
        $("#checkoutForm").find("#billing_first_name").val()
    );
    var billing_last_name = $.trim(
        $("#checkoutForm").find("#billing_last_name").val()
    );
    var billing_address = $.trim(
        $("#checkoutForm").find("#billing_address").val()
    );
    var billing_apartment = $.trim(
        $("#checkoutForm").find("#billing_apartment").val()
    );
    var billing_country = $.trim(
        $("#checkoutForm").find("#billing_country").val()
    );
    var billing_state = $.trim($("#checkoutForm").find("#billing_state").val());
    var billing_city = $.trim($("#checkoutForm").find("#billing_city").val());
    var billing_pincode = $.trim(
        $("#checkoutForm").find("#billing_pincode").val()
    );
    var billing_mobile = $.trim(
        parseInt($("#checkoutForm").find("#billing_mobile").val())
    );
    if (first_name == "") {
    ...
    if (!$("#checkoutForm").find("#samebilling").is(":checked")) {
        if (billing_first_name == "") {
            $("#checkoutForm #billing_first_name")
                .parent("div")
                .find("span")
                .html("Please enter first name");
            flag = false;
        ...
        if (billing_last_name == "") {
            $("#checkoutForm #billing_last_name")
                .parent("div")
                .find("span")
                .html("Please enter last name");
            flag = false;
        ...
        if (billing_apartment == "") {
            $("#checkoutForm #billing_apartment")
                .parent("div")
                .find("span")
                .html("Please enter your apartment.");
            flag = false;
    ...
    var shipping_method = $("#deliveryMethodSection")
        .find(".shippingMethod:checked")
        .val();
    if (flag) {
