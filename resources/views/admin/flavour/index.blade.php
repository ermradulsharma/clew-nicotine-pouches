    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="search-form">
                <form id="searchForm" name="searchForm" class="form-inline" action="{{ route('admin.flavour.index') }}">
                    ...
                    <button type="button" class="btn btn-default btn-sm" onClick="window.location='{{ route('admin.flavour.index') }}'">Reset</button>

                    <a href="{{ route('admin.flavour.create') }}" class="btn btn-default btn-sm pull-right">Add</a>
                </form>
            </div>
            <!-- OVERVIEW -->
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr id="row-{{ $data->id }}">
                                ...
                                <td>{{ $data->title }}</td>
                                <td>
									<form id="positionForm" name="positionForm" action="{{ route('admin.flavour.position') }}" method="post">
                                ...
                                <td class="action">
                                    <a href="{{route('admin.flavour.edit', $data->id)}}" class="text-info" data-toggle="tooltip" data-placement="bottom" title="Edit">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <a href="javascript:void(0)" id="publishData-{{ $data->id }}" class="publishData {{ $data->status? 'text-success': 'text-warning' }}" data-id="{{ $data->id }}" data-alias="Flavour" data-toggle="tooltip" data-placement="bottom" title="Status">
                                        <i class="fa fa-{{ $data->status?'check':'times' }}-circle"></i>
                                    </a>
                                    {{--<a href="javascript:void(0)" class="deleteData text-danger" data-id="{{ $data->id }}" data-alias="Flavour" data-toggle="tooltip" data-placement="bottom" title="Delete">
